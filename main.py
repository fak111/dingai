...
def get_user_data_dir():
    """
    获取用户数据目录（用于存储下载文件、日志、配置文件备份等可写数据）。
    ...
    if not os.path.exists(app_data_dir):
        try:
            ...
            # 使用 print 而不是 logger，因为这是在模块初始化阶段
            print(f"⚠️  无法创建用户数据目录 {app_data_dir}: {e}")
            # 回退到当前目录
            if hasattr(sys, "_MEIPASS"):
                app_data_dir = os.path.dirname(sys.executable)
            else:
                app_data_dir = os.path.split(os.path.realpath(sys.argv[0]))[0]

    return app_data_dir


# 配置文件路径：优先使用用户数据目录（可写），如果不存在则使用资源路径（只读）
user_data_dir = get_user_data_dir()
config_dir = os.path.join(user_data_dir, "config")
backup_dir = os.path.join(user_data_dir, "backup_config")
...
url_config_file = os.path.join(config_dir, "URL_config.ini")

# 如果用户数据目录中没有配置文件，尝试从资源路径复制（首次运行）
# 注意：这里使用 print 而不是 logger，因为这是在模块初始化阶段
if not os.path.exists(config_file):
    resource_config = get_resource_path("config/config.ini")
    if os.path.exists(resource_config):
        try:
            import shutil

            shutil.copy2(resource_config, config_file)
            print(f"✅ 已从资源目录复制配置文件到: {config_file}")
        except Exception as e:
            print(f"⚠️  无法复制配置文件: {e}")

if not os.path.exists(url_config_file):
    resource_url_config = get_resource_path("config/URL_config.ini")
    if os.path.exists(resource_url_config):
        try:
            import shutil

            shutil.copy2(resource_url_config, url_config_file)
            print(f"✅ 已从资源目录复制URL配置文件到: {url_config_file}")
        except Exception as e:
            print(f"⚠️  无法复制URL配置文件: {e}")

text_encoding = "utf-8-sig"
rstr = r"[\/\\\:\*\？?\"\<\>\|&#.。,， ~！· ]"
# 下载目录使用用户数据目录
default_path = os.path.join(user_data_dir, "downloads")
        ...
        try:
            ...
            if len(recording) == 0:
                ...
                for recording_live in no_repeat_recording:
                    ...
                    print(
                        f"{recording_live}[{qa}] 正在录制中 {str(have_record_time).split('.')[0]}"
                    )
...
def update_file(
    file_path: str, old_str: str, new_str: str, start_str: str = None
) -> str | None:
...
def segment_video(
    ...
    try:
        if (
            os.path.exists(converts_file_path)
            and os.path.getsize(converts_file_path) > 0
        ):
            ffmpeg_command = [
                "ffmpeg",
                "-i",
...
def generate_subtitles(
    record_name: str, ass_filename: str, sub_format: str = "srt"
) -> None:
    index_time = 0
    ...
    while True:
        ...
        with open(
            f"{ass_filename}.{sub_format.lower()}", "a", encoding=text_encoding
        ) as f:
            f.write(txt)
...
def push_message(record_name: str, live_url: str, content: str) -> None:
    ...
    push_functions = {
        ...
        "钉钉": lambda: dingtalk(
            dingtalk_api_url, content, dingtalk_phone_num, dingtalk_is_atall
        ),
    ...
    for platform, func in push_functions.items():
        if platform in live_status_push.upper():
            try:
                ...
                color_obj.print_colored(
                    f"直播消息推送到{platform}失败: {e}", color_obj.RED
                )
...
def run_script(command: str) -> None:
    try:
        ...
        logger.error(e)
        logger.error(
            "脚本无执行权限!, 若是Linux环境, 请先执行:chmod +x your_script.sh 授予脚本可执行权限"
        )
    except OSError as e:
        logger.error(e)
        logger.error(
            "Please add `#!/bin/bash` at the beginning of your bash script file."
        )
...
def direct_download_stream(
    source_url: str, save_path: str, record_name: str, live_url: str, platform: str
) -> bool:
    try:
...
def select_source_url(link, stream_info):
    if is_flv_preferred_platform(link):
        ...
        if codec and codec[0] == "h265":
            logger.warning(
                "FLV is not supported for h265 codec, use HLS source instead"
            )
        else:
            ...
            while True:
                try:
                    ...
                    if record_url.find("douyin.com/") > -1:
                        ...
                        with semaphore:
                            if (
                                "v.douyin.com" not in record_url
                                and "/user/" not in record_url
                            ):
                            ...
                            port_info = asyncio.run(
                                stream.get_douyin_stream_url(
                                    json_data, record_quality, proxy_address
                                )
                            )
                        ...
                        with semaphore:
                            if global_proxy or proxy_address:
                                ...
                                port_info = asyncio.run(
                                    stream.get_tiktok_stream_url(
                                        json_data, record_quality, proxy_address
                                    )
                                )
                            else:
                                logger.error(
                                    "错误信息: 网络异常，请检查网络是否能正常访问TikTok平台"
                                )
                        ...
                        with semaphore:
                            ...
                            port_info = asyncio.run(
                                stream.get_kuaishou_stream_url(
                        ...
                        record_url.find("www.bigo.tv/") > -1
                        or record_url.find("slink.bigovideo.tv/") > -1
                    ):
                        platform = "Bigo直播"
                        with semaphore:
                            port_info = asyncio.run(
                                spider.get_bigo_stream_url(
                                    record_url,
                                    proxy_addr=proxy_address,
                                    cookies=bigo_cookie,
                                )
                        ...
                        with semaphore:
                            port_info = asyncio.run(
                                spider.get_blued_stream_url(
                                    record_url,
                                    proxy_addr=proxy_address,
                                    cookies=blued_cookie,
                                )
                            )

                    elif (
                        record_url.find("sooplive.co.kr/") > -1
                        or record_url.find("sooplive.com/") > -1
                    ):
                        platform = "SOOP"
                        with semaphore:
                            if global_proxy or proxy_address:
                                ...
                                port_info = asyncio.run(
                                    stream.get_stream_url(
                                        json_data, record_quality, spec=True
                                    )
                                )
                            else:
                                logger.error(
                                    "错误信息: 网络异常，请检查本网络是否能正常访问SOOP平台"
                                )
                        ...
                        with semaphore:
                            ...
                            port_info = asyncio.run(
                                stream.get_netease_stream_url(json_data, record_quality)
                            )
                        ...
                        with semaphore:
                            if global_proxy or proxy_address:
                                ...
                                logger.error(
                                    "错误信息: 网络异常，请检查本网络是否能正常访问PandaTV直播平台"
                                )
                        ...
                        with semaphore:
                            if global_proxy or proxy_address:
                                ...
                                logger.error(
                                    "错误信息: 网络异常，请检查本网络是否能正常访问WinkTV直播平台"
                                )

                    elif (
                        record_url.find("www.flextv.co.kr/") > -1
                        or record_url.find("www.ttinglive.com/") > -1
                    ):
                        platform = "FlexTV"
                        with semaphore:
                            if global_proxy or proxy_address:
                                ...
                                logger.error(
                                    "错误信息: 网络异常，请检查本网络是否能正常访问FlexTV直播平台"
                                )
                        ...
                        with semaphore:
                            if global_proxy or proxy_address:
                                ...
                                logger.error(
                                    "错误信息: 网络异常，请检查本网络是否能正常访问PopkonTV直播平台"
                                )
                        ...
                        with semaphore:
                            ...
                            port_info = asyncio.run(
                                stream.get_stream_url(
                                    json_data, record_quality, spec=False
                                )
                            )
                        ...
                        with semaphore:
                            ...
                            port_info = asyncio.run(
                                stream.get_stream_url(json_data, record_quality)
                            )
                        ...
                        with semaphore:
                            if global_proxy or proxy_address:
                                ...
                                logger.error(
                                    "错误信息: 网络异常，请检查本网络是否能正常访问TwitchTV直播平台"
                                )
                            ...
                            with semaphore:
                                port_info = asyncio.run(
                                    spider.get_liveme_stream_url(
                                        url=record_url,
                                        proxy_addr=proxy_address,
                                        cookies=liveme_cookie,
                                    )
                                )
                        else:
                            logger.error(
                                "错误信息: 网络异常，请检查本网络是否能正常访问LiveMe直播平台"
                            )
                        ...
                        record_url.find("live.acfun.cn/") > -1
                        or record_url.find("m.acfun.cn/") > -1
                    ):
                        platform = "Acfun"
                        with semaphore:
                            json_data = asyncio.run(
                        ...
                        with semaphore:
                            port_info = asyncio.run(
                                spider.get_pplive_stream_url(
                                    url=record_url,
                                    proxy_addr=proxy_address,
                                    cookies=huamao_cookie,
                                )
                            )

                    elif (
                        record_url.find("live.shopee") > -1
                        or record_url.find("shp.ee/") > -1
                    ):
                        platform = "shopee"
                        with semaphore:
                            port_info = asyncio.run(
                        ...
                        record_url.find("www.youtube.com/") > -1
                        or record_url.find("youtu.be/") > -1
                    ):
                        platform = "Youtube"
                        with semaphore:
                            json_data = asyncio.run(
                        ...
                        record_url.find("3.cn") > -1 or record_url.find("m.jd.com") > -1
                    ):
                        platform = "京东直播"
                        with semaphore:
                            port_info = asyncio.run(
                        ...
                        with semaphore:
                            if global_proxy or proxy_address:
                                ...
                                logger.error(
                                    "错误信息: 网络异常，请检查本网络是否能正常访问faceit直播平台"
                                )

                    elif (
                        record_url.find("www.miguvideo.com") > -1
                        or record_url.find("m.miguvideo.com") > -1
                    ):
                        platform = "咪咕直播"
                        with semaphore:
                            port_info = asyncio.run(
                    ...
                    if not port_info.get("anchor_name", ""):
                        print(
                            f"序号{count_variable} 网址内容获取失败,进行重试中...获取失败的地址是:{url_data}"
                        )
                        with max_request_lock:
                            error_count += 1
                        ...
                        push_at = datetime.datetime.today().strftime(
                            "%Y-%m-%d %H:%M:%S"
                        )
                        if port_info["is_live"] is False:
                            ...
                            if real_url:
                                ...
                                if live_title:
                                    ...
                                    title_in_name = (
                                        live_title + "_" if filename_by_title else ""
                                    )
                                ...
                                try:
                                    ...
                                    if folder_by_title and port_info.get("title"):
                                        if folder_by_time:
                                            ...
                                            full_path = (
                                                f"{full_path}/{now[:10]}_{live_title}"
                                            )
                                ...
                                if platform != "自定义录制直播":
                                    if enable_https_recording and real_url.startswith(
                                        "http://"
                                    ):
                                ...
                                rec_info = (
                                    f"\r{anchor_name} 准备开始录制视频: {full_path}"
                                )
                                if show_url:
                                    re_plat = (
                                        "WinkTV",
                                ...
                                if is_flv_preferred_platform(
                                    record_url
                                ) and port_info.get("flv_url"):
                                    codec = utils.get_query_params(
                                        port_info["flv_url"], "codec"
                                    )
                                    if codec and codec[0] == "h265":
                                        logger.warning(
                                ...
                                if only_audio_record or any(
                                    i in record_save_type for i in ["MP3", "M4A"]
                                ):
                                    try:
                                        now = time.strftime(
                                        ...
                                        extension = (
                                            "mp3"
                                            if "m4a" not in record_save_type.lower()
                                            else "m4a"
                                        )
                                        name_format = (
                                            "_%03d" if split_video_by_time else ""
                                        )
                                        save_file_path = (
                                        ...
                                        if split_video_by_time:
                                            print(
                                                f"\r{anchor_name} 准备开始录制音频: {save_file_path}"
                                            )
                                ...
                                if only_flv_record:
                                    logger.info(
                                        f"Use Direct Downloader to Download FLV Stream: {record_url}"
                                    )
                                    filename = (
                                        anchor_name + f"_{title_in_name}" + now + ".flv"
                                    )
                                    save_file_path = f"{full_path}/{filename}"
                                    print(f"{rec_info}/{filename}")

                                    subs_file_path = save_file_path.rsplit(
                                        ".", maxsplit=1
                                    )[0]
                                    subs_thread_name = (
                                        f"subs_{Path(subs_file_path).name}"
                                    )
                                    if create_time_file:
                                        create_var[subs_thread_name] = threading.Thread(
                                    ...
                                    filename = (
                                        anchor_name + f"_{title_in_name}" + now + ".mkv"
                                    )
                                    print(f"{rec_info}/{filename}")
                                    save_file_path = full_path + "/" + filename
                                    ...
                                    filename = (
                                        anchor_name + f"_{title_in_name}" + now + ".mp4"
                                    )
                                    print(f"{rec_info}/{filename}")
                                    save_file_path = full_path + "/" + filename
                ...
                if error_count > 20:
                    x = x + 60
                    color_obj.print_colored(
                        "\r瞬时错误太多,延迟加60秒", color_obj.YELLOW
                    )
...
# --------------------------初始化程序-------------------------------------
# 将启动相关的初始化移到函数中，只在直接运行时执行
def initialize_main_program():
    """初始化主程序 - 只在直接运行时调用"""
    # 注册 signal handler（只能在主线程中注册）
...
# 初始化配置变量（无论作为模块导入还是直接运行都需要）
video_save_path = read_config_value(config, "录制设置", "直播保存路径(不填则默认)", "")
folder_by_author = options.get(
    read_config_value(config, "录制设置", "保存文件夹是否以作者区分", "是"),
    False,
)
folder_by_time = options.get(
    read_config_value(config, "录制设置", "保存文件夹是否以时间区分", "否"),
    False,
)
folder_by_title = options.get(
    read_config_value(config, "录制设置", "保存文件夹是否以标题区分", "否"),
    False,
)
filename_by_title = options.get(
    read_config_value(config, "录制设置", "保存文件名是否包含标题", "否"), False
)
clean_emoji = options.get(
    read_config_value(config, "录制设置", "是否去除名称中的表情符号", "是"),
    True,
)
video_save_type = read_config_value(
    config, "录制设置", "视频保存格式ts|mkv|flv|mp4|mp3音频|m4a音频", "ts"
)
video_record_quality = read_config_value(
    config, "录制设置", "原画|超清|高清|标清|流畅", "原画"
)
use_proxy = options.get(
    read_config_value(config, "录制设置", "是否使用代理ip(是/否)", "是"), False
)
...
local_delay_default = int(
    read_config_value(config, "录制设置", "排队读取网址时间(秒)", 0)
)
loop_time = options.get(
    read_config_value(config, "录制设置", "是否显示循环秒数", "否"), False
)
show_url = options.get(
    read_config_value(config, "录制设置", "是否显示直播源地址", "否"), False
)
split_video_by_time = options.get(
    read_config_value(config, "录制设置", "分段录制是否开启", "否"), False
)
enable_https_recording = options.get(
    read_config_value(config, "录制设置", "是否强制启用https录制", "否"), False
)
disk_space_limit = float(
    read_config_value(config, "录制设置", "录制空间剩余阈值(gb)", 1.0)
)
split_time = str(read_config_value(config, "录制设置", "视频分段时间(秒)", 1800))
converts_to_mp4 = options.get(
    read_config_value(config, "录制设置", "录制完成后自动转为mp4格式", "否"),
    False,
)
converts_to_h264 = options.get(
    read_config_value(config, "录制设置", "mp4格式重新编码为h264", "否"), False
)
delete_origin_file = options.get(
    read_config_value(config, "录制设置", "追加格式后删除原文件", "否"), False
)
create_time_file = options.get(
    read_config_value(config, "录制设置", "生成时间字幕文件", "否"), False
)
is_run_script = options.get(
    read_config_value(config, "录制设置", "是否录制完成后执行自定义脚本", "否"),
    False,
)
custom_script = (
    read_config_value(config, "录制设置", "自定义脚本执行命令", "")
    if is_run_script
    else None
)
...
enable_proxy_platform_list = (
    enable_proxy_platform.replace("，", ",").split(",")
    if enable_proxy_platform
    else None
)
extra_enable_proxy = read_config_value(
    config, "录制设置", "额外使用代理录制的平台(逗号分隔)", ""
)
...
dingtalk_phone_num = read_config_value(
    config, "推送配置", "钉钉通知@对象(填手机号)", ""
)
dingtalk_is_atall = options.get(
    read_config_value(config, "推送配置", "钉钉通知@全体(是/否)", "否"), False
)
...
open_smtp_ssl = options.get(
    read_config_value(config, "推送配置", "是否使用SMTP服务SSL加密(是/否)", "是"),
    True,
)
...
push_message_title = read_config_value(
    config, "推送配置", "自定义推送标题", "直播间状态更新通知"
)
begin_push_message_text = read_config_value(
    config, "推送配置", "自定义开播推送内容", ""
)
over_push_message_text = read_config_value(config, "推送配置", "自定义关播推送内容", "")
disable_record = options.get(
    read_config_value(config, "推送配置", "只推送通知不录制(是/否)", "否"),
    False,
)
push_check_seconds = int(
    read_config_value(config, "推送配置", "直播推送检测频率(秒)", 1800)
)
begin_show_push = options.get(
    read_config_value(config, "推送配置", "开播推送开启(是/否)", "是"), True
)
over_show_push = options.get(
    read_config_value(config, "推送配置", "关播推送开启(是/否)", "否"), False
)
...
twitcasting_account_type = read_config_value(
    config, "账号密码", "twitcasting账号类型", "normal"
)
...
# 解析命令行参数和主循环（只在直接运行时执行）
if __name__ == "__main__":
    # 只在直接运行时才执行初始化
    initialize_main_program()

    parser = argparse.ArgumentParser(description="直播录制工具")
    parser.add_argument(
        "url", nargs="?", help="直播间URL地址（可选，如果提供则直接使用此URL）"
    )
    args = parser.parse_args()

    # 如果提供了命令行URL，则使用命令行URL，否则使用配置文件
    command_line_url = args.url.strip() if args.url and args.url.strip() else None

    # 如果提供了命令行URL，先写入配置文件
    if command_line_url:
        try:
            with open(url_config_file, "w", encoding=text_encoding) as file:
                file.write(command_line_url)
            print(f"📺 使用命令行指定的URL: {command_line_url}")
        except OSError as err:
            logger.error(f"写入URL配置文件失败: {err}")
    ...
    while True:
        ...
        try:
            ...
            with open(
                url_config_file, "r", encoding=text_encoding, errors="ignore"
            ) as file:
                for origin_line in file:
                    ...
                    if len(line_spilt) > 2:
                        line = update_file(
                            url_config_file,
                            line,
                            f"{line_spilt[0]}主播: {line_spilt[-1]}",
                        )
                    ...
                    if "live.shopee." in url_host or ".shp.ee" in url_host:
                        url_host = (
                            "live.shopee." if "live.shopee." in url_host else ".shp.ee"
                        )

                    if url_host in platform_host or any(
                        ext in url for ext in (".flv", ".m3u8")
                    ):
                        ...
                        if "xiaohongshu" in url:
                            ...
                            if host_id:
                                new_url = (
                                    url.split("?")[0] + f"?host_id={host_id.group(1)}"
                                )
                                url = update_file(
                                    url_config_file, old_str=url, new_str=new_url
                                )
            ...
            while len(need_update_line_list):
                ...
                if replace_words[0] != replace_words[1]:
                    ...
                    update_file(
                        url_config_file,
                        old_str=replace_words[0],
                        new_str=new_word,
                        start_str=start_with,
                    )
            ...
            if len(text_no_repeat_url) > 0:
                for url_tuple in text_no_repeat_url:
                    ...
                    if url_tuple[1] not in running_list:
                        print(
                            f"\r{'新增' if not first_start else '传入'}地址: {url_tuple[1]}"
                        )
                        monitoring += 1
                        args = [url_tuple, monitoring]
                        create_var[f"thread_{monitoring}"] = threading.Thread(
                            target=start_record, args=args
                        )
